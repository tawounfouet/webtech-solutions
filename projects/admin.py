...
class ClientAdmin(admin.ModelAdmin):
    list_display = ['name', 'logo_preview', 'logo_white_preview', 'website_link', 'is_active', 'projects_count']
    list_filter = ['is_active', 'created_at']
    search_fields = ['name', 'description']
    prepopulated_fields = {'slug': ('name',)}
    readonly_fields = ['created_at', 'updated_at', 'logo_preview_large', 'logo_white_preview_large']
    
    fieldsets = (
        ('Informations générales', {
            'fields': ('name', 'slug', 'description', 'website', 'is_active')
        }),
        ('Logo principal', {
            'fields': (
                'logo_preview_large',
    ...
    def projects_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:projects_project_changelist') + f'?client__id__exact={obj.id}'
            return format_html(
    ...
    def mark_clients_as_active(modeladmin, request, queryset):
        ...
        modeladmin.message_user(
            request,
            f'{updated} client(s) marqué(s) comme actif(s).',
            level='success'
        )
    ...
    def mark_clients_as_inactive(modeladmin, request, queryset):
        ...
        modeladmin.message_user(
            request,
            f'{updated} client(s) marqué(s) comme inactif(s).',
            level='success'
        )
...
class ProjectAdmin(admin.ModelAdmin):
    list_display = [
        'title', 'featured_image_preview', 'client', 'status_colored', 
        'categories_display', 'is_featured', 'is_published', 'created_at'
    ]
    list_filter = [
        'status', 'is_featured', 'is_published', 'categories', 
        'client', 'created_at'
    ]
    ...
    readonly_fields = [
        ...
        'featured_image_preview_large', 'thumbnail_preview_large'
    ]
    ...
    inlines = [ProjectImageInline, ProjectTeamMemberInline, ProjectTestimonialInline, ProjectMetricsInline]
    ...
    def categories_display(self, obj):
        ...
        if categories.count() > 3:
            result += format_html('<span style="color: #666;">+{}</span>', categories.count() - 3)
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('client').prefetch_related('categories')
    ...
    def mark_as_published(modeladmin, request, queryset):
        ...
        modeladmin.message_user(
            request,
            f'{updated} projet(s) marqué(s) comme publié(s).',
            level='success'
        )
...
class ProjectTeamMemberAdmin(admin.ModelAdmin):
    ...
    search_fields = ['user__username', 'user__first_name', 'user__last_name', 'project__title', 'role']
...
class ProjectTestimonialAdmin(admin.ModelAdmin):
    list_display = ['client_name', 'project', 'rating_stars', 'is_featured', 'created_at'
]
